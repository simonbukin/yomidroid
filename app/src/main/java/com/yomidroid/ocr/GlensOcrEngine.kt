 ...
 * Ported from the Yomihon project's GlensOcrEngine.
 */
class GlensOcrEngine : OcrEngine {

    override val engineId = "glens"
    override val displayName = "Google Lens (Online)"
    ...
    override suspend fun processImage(bitmap: Bitmap): Result<List<OcrResult>> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                val preparedImage = prepareImage(bitmap)
                val payload = buildRequestPayload(preparedImage)
                val responseBytes = executeRequest(payload)
                val results = parseResponseToOcrResults(responseBytes, bitmap.width, bitmap.height)

                val totalMs = (System.nanoTime() - startTime) / 1_000_000
                Log.i(TAG, "Glens OCR total time: ${totalMs}ms, ${results.size} regions")
                results
            }.onFailure { e ->
    ...
    private fun parseResponseToOcrResults(
        ...
        return filteredLines.mapNotNull { line -> line.toOcrResult(bitmapWidth, bitmapHeight) }
    }

    private fun ParsedLine.toOcrResult(bitmapWidth: Int, bitmapHeight: Int): OcrResult? {
        if (text.isBlank() || width <= 0f || height <= 0f) return null

        val left = ((centerX - width / 2f) * bitmapWidth).toInt().coerceIn(0, bitmapWidth)
        val top = ((centerY - height / 2f) * bitmapHeight).toInt().coerceIn(0, bitmapHeight)
        val right = ((centerX + width / 2f) * bitmapWidth).toInt().coerceIn(0, bitmapWidth)
        val bottom = ((centerY + height / 2f) * bitmapHeight).toInt().coerceIn(0, bitmapHeight)
        val lineBounds = Rect(left, top, right, bottom)
        if (lineBounds.width() <= 0 || lineBounds.height() <= 0) return null

        val cleaned = textPostprocessor.postprocess(text).trim()
        if (cleaned.isBlank()) return null

        val charBounds = OcrUtils.synthesizeCharBounds(cleaned, lineBounds)
        if (charBounds.size != cleaned.length) return null

        return OcrResult(cleaned, lineBounds, charBounds)
    }
    ...
    private fun parseBoundingBox(bboxBytes: ByteArray): BoundingBoxData {
        ...
        while (reader.hasRemaining()) {
            ...
            val wireType = tag and WIRE_TYPE_MASK
            when (field) {
                BOUNDING_BOX_CENTER_X -> if (wireType == WIRE_TYPE_32BIT) centerX = reader.readFloat() else reader.skipField(wireType)
                BOUNDING_BOX_CENTER_Y -> if (wireType == WIRE_TYPE_32BIT) centerY = reader.readFloat() else reader.skipField(wireType)
                BOUNDING_BOX_WIDTH -> if (wireType == WIRE_TYPE_32BIT) width = reader.readFloat() else reader.skipField(wireType)
                BOUNDING_BOX_HEIGHT -> if (wireType == WIRE_TYPE_32BIT) height = reader.readFloat() else reader.skipField(wireType)
                else -> reader.skipField(wireType)
            }
        }
    ...
    companion object {
        private const val TAG = "GlensOcr"
